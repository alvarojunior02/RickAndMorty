...
const SearchResultScreen = ({route}: any) => {
    ...
    function getCharactersByName(search: string, page: number) {
        try {
            api.get('/character', {
                params: {
    ...
    useEffect(() => {
        getCharactersByName(route.params.search, 1);
    }, [])
    ...
    return (
        <>
            <View style={styles.container}>
                ...
                <Text style={styles.textTip}>Searching by: {route.params.search}</Text>
                {
                    characters.length === 0 ?
                        <LoaderSpinner />
                    : (
                ...
                <View style={styles.containerPagination}>
                    <TouchableOpacity
                        ...
                        onPress={() => {
                            getCharactersByName(route.params.search, prev);
                            setPrev(prev - 1);
                            setNext(next - 1);
                        }}
                    ...
                    <TouchableOpacity 
                        ...
                        onPress={() => {
                            getCharactersByName(route.params.search, next);
                            setPrev(prev + 1);
                            setNext(next + 1);
                        }}
                        style={styles.buttonPagination}
